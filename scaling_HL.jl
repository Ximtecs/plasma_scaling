...
mutable struct ScalingHL
    ...
    function ScalingHL(base_units, number_density=1e15, length=1e8, temperature=1e6, temperature_e=1e6, temperature_p=1e6, B_flux=1e1, length_scale=3e8, mass_density_scale=1e-13, time_scale=1.0, temperature_scale=1e6)
...
function print_fundamentals(obj::ScalingHL)
    ...
    @printf("%-50s = % .4e % .4e % .4e\n", "           Vacuum Permittivity [ s^2 cm^-2 ]", obj.eps_0_real, obj.eps_0_scaling, obj.eps_0_code)
    @printf("%-50s = % .4e % .4e % .4e\n", "           Vacuum Permeability [  ]", obj.mu_0_real, obj.mu_0_scaling, obj.mu_0_code)
